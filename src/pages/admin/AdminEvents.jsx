      ...
      {isLoading ? (
        ...
        <div className="bg-[#0d0d0d] border border-white/[0.06]">
          <div className="px-6 py-3 border-b border-white/[0.06] flex gap-4">
            <p className="font-heading text-[8px] tracking-[0.3em] uppercase text-white/20 w-52 flex-shrink-0">Event</p>
            <p className="font-heading text-[8px] tracking-[0.3em] uppercase text-white/20 w-28 flex-shrink-0">ID</p>
            <p className="font-heading text-[8px] tracking-[0.3em] uppercase text-white/20 w-24 flex-shrink-0">Date</p>
          ...
          <div className="divide-y divide-white/[0.04]">
            ...
            {events.map((event) => (
              <div key={event.id} className="px-6 py-4 flex gap-4 items-center hover:bg-white/[0.02] transition-colors">
                ...
                <p className="w-28 flex-shrink-0 text-white/20 text-[10px] font-mono truncate cursor-pointer hover:text-white/50 transition-colors" title={event.id} onClick={() => navigator.clipboard.writeText(event.id)}>{event.id?.slice(0, 8)}…</p>
                <p className="w-24 flex-shrink-0 text-white/30 text-xs font-light">
                  {event.date ? format(new Date(event.date), "MMM d, yyyy") : "—"}
                </p>
                <p className="w-28 flex-shrink-0 text-white/30 text-xs font-light">{event.price || "—"}</p>
                ...
                <p className="w-12 flex-shrink-0 text-white/20 text-xs text-center">{event.display_order}</p>
